        ...
        <div v-else>
            ...
            <div class="question_bank">
                ...
                <form class="w-100" @submit.prevent="submitMock">
                    ...
                    <div v-for="(questionGroup, index) in questionGroups" class="p-3 rounded mb-5" v-if="questionGroup.questions.length > 0">
                        <h3 v-if="questionGroup.title != 'Select'">{{questionGroup.title}}</h3>
                             <div v-for="(question, index) in questionGroup.questions" class="card p-3 rounded mb-5 shadow-sm" >
                                <div class="d-flex">
                                    <div class="px-3">
                                        <h6 class="mt-3">{{question.questionNumber}}</h6>
                                    </div>
...
export default {
    ...
    methods: {
        async fetchQuestions(){
                let url;
                if (localStorage.getItem("examid") == null || localStorage.getItem("examid") == ""){
                    url = this.appURL+`/get-questions-fresh/${this.user}/${this.title}`
                }else{
                    ...
                    for(let i = 0; i < theQuestionGroups.length; i++){
                        ...
                        this.questions.forEach((question)=>{
                            if (localStorage.getItem("examid") == null) {
                                if (theQuestionGroups[i].id == question.question.group_id){
                                ...
                                if (theQuestionGroups[i].group_id == question.question.group_id){
                                    question["questionNumber"] = questionNumber
                                    questionNumber ++
                                    thisGroupQuestions.push(question)
                                    console.log(question);
        ...
        async countDown(){
            if (this.sec_left > 0){
                let url = this.appURL+`/update-time/${this.time_left}/${this.exam.id}`
                let response = await axios.get(url)
                ...
                if (response.data.total_sec == 1) {
                    ...
                    window.location.assign(this.appURL+`/submit/${this.exam.exam_id}`)
                }
            }
        },
        async answerClick(question, option){
            let url = this.appURL+`/answer/${question}/${option}/${this.exam.id}`
            let response = await axios.get(url)
            this.fetchQuestions()
        },
